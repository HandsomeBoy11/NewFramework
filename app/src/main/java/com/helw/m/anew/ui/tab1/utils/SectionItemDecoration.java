...
public class SectionItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public SectionItemDecoration(Context context, List<String> data) {
        this.mData = data;
...
//        mBgColor = context.getResources().getColor(typedValue.resourceId);
        mBgColor = context.getResources().getColor(R.color.white_light);
...
//        mSectionHeight = context.getResources().getDimensionPixelSize(typedValue.resourceId);
        mSectionHeight = context.getResources().getDimensionPixelSize(R.dimen.sp_biggest);
...
//        mTextSize = context.getResources().getDimensionPixelSize(typedValue.resourceId);
        mTextSize = context.getResources().getDimensionPixelSize(R.dimen.sp_small);
...
//        mTextColor = context.getResources().getColor(typedValue.resourceId);
        mTextColor = context.getResources().getColor(R.color.tv_black);
            ...
            if (mData != null && !mData.isEmpty() && position <= mData.size() - 1 && position > -1) {
                if (position == 0) {
                    ...
                    if (null != mData.get(position)
                            && !mData.get(position).equals(mData.get(position - 1))) {
                        drawSection(c, left, right, child, params, position);
                    }
                }
            }
    ...
    private void drawSection(Canvas c, int left, int right, View child,
        ...
        mTextPaint.getTextBounds(mData.get(position),
                0,
                mData.get(position).length(),
                mBounds);
        c.drawText(mData.get(position),
    ...
    public void onDrawOver(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        String section = mData.get(pos);
        View child = parent.findViewHolderForLayoutPosition(pos).itemView;

        boolean flag = false;
        if ((pos + 1) < mData.size()) {
            if (null != section && !section.equals(mData.get(pos + 1))) {
